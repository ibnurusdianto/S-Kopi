            ...
            <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans">
                <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-sm border border-gray-100">
                    <div className="text-center mb-8">
                        <h1 className="text-2xl font-black text-amber-700">Admin Login</h1>
                    ...
                    <form onSubmit={handleLogin} className="space-y-5">
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-amber-500" 
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row font-sans text-gray-900">
            ...
            <aside className="w-full md:w-64 bg-white border-r border-gray-200 shrink-0 hidden md:block">
                <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                    <div>
                        <h1 className="text-xl font-black text-amber-700">Admin Panel</h1>
                ...
                <nav className="p-4 space-y-1">
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link key={item.href} href={item.href} className={`block px-4 py-3 rounded-xl text-sm font-semibold transition-colors ${isActive ? "bg-amber-50 text-amber-700" : "text-gray-600 hover:bg-gray-50"}`}>
                                {item.name}
                            </Link>
                        );
                    })}
            ...
            <header className="md:hidden bg-white border-b border-gray-200 p-4 flex justify-between items-center">
                <h1 className="text-lg font-black text-amber-700">Admin Panel</h1>
            ...
            <div className="md:hidden bg-white border-b border-gray-200 overflow-x-auto">
                <div className="flex p-2 space-x-2 w-max">
                    {navItems.map((item) => {
                        const isActive = pathname === item.href;
                        return (
                            <Link key={item.href} href={item.href} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${isActive ? "bg-amber-600 text-white" : "bg-gray-100 text-gray-600"}`}>
                                {item.name}
                            </Link>
                        );
                    })}
