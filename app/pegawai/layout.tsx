            ...
            <div className="min-h-screen bg-gray-50 dark:bg-neutral-950 flex items-center justify-center p-4 font-sans text-gray-900 dark:text-white">
                <div className="bg-white dark:bg-neutral-900 p-8 rounded-3xl shadow-xl w-full max-w-sm border border-gray-100 dark:border-neutral-800">
                    <div className="text-center mb-8">
                        <h1 className="text-2xl font-black text-green-700 dark:text-green-500">Pegawai Login</h1>
                        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Sasa Kopi Dashboard</p>
                    </div>
                    <form onSubmit={handleLogin} className="space-y-5">
                        {error && (
                            <div className="p-3 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-sm font-semibold rounded-xl text-center border border-red-100 dark:border-red-900/50">
                                {error}
                            </div>
                        )}
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full bg-gray-50 dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-900 dark:text-white" 
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-neutral-950 flex flex-col md:flex-row font-sans text-gray-900 dark:text-white">
            ...
            <aside className="w-full md:w-64 bg-white dark:bg-neutral-900 border-r border-gray-200 dark:border-neutral-800 shrink-0 hidden md:block">
                <div className="p-6 border-b border-gray-100 dark:border-neutral-800 flex justify-between items-center">
                    <div>
                        <h1 className="text-xl font-black text-green-700 dark:text-green-500">Pegawai Panel</h1>
                ...
                <nav className="p-4 space-y-1">
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link key={item.href} href={item.href} className={`block px-4 py-3 rounded-xl text-sm font-semibold transition-colors ${isActive ? "bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-500" : "text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-neutral-800 dark:hover:text-white"}`}>
                                {item.name}
                            </Link>
                        );
                    })}
            ...
            <header className="md:hidden bg-white dark:bg-neutral-900 border-b border-gray-200 dark:border-neutral-800 p-4 flex justify-between items-center">
                <h1 className="text-lg font-black text-green-700 dark:text-green-500">Pegawai Panel</h1>
            ...
            <div className="md:hidden bg-white dark:bg-neutral-900 border-b border-gray-200 dark:border-neutral-800 overflow-x-auto">
                <div className="flex p-2 space-x-2 w-max">
                    {navItems.map((item) => {
                        const isActive = pathname === item.href;
                        return (
                            <Link key={item.href} href={item.href} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${isActive ? "bg-green-600 text-white" : "bg-gray-100 dark:bg-neutral-800 text-gray-600 dark:text-gray-300"}`}>
                                {item.name}
                            </Link>
                        );
                    })}
